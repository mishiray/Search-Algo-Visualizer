...
public class Controller {
    ...
    private final SimpleObjectProperty<Thread> solveThread = new SimpleObjectProperty<>();
     ...
     * Node space events
     * */
    @FXML private void onGraphSpacePressed(MouseEvent e){
        //Restrict to left click
        if(e.isPrimaryButtonDown()){
            node1 = createAndAddNode(e.getX(), e.getY());
        }
    }
    @FXML private void onGraphSpaceDragDetected(MouseEvent e) {
        if(e.isPrimaryButtonDown()) {
    ...
    @FXML private void onGraphSpaceDragged(MouseEvent e) {
        if(node2 != null){
            node2.setLayoutX(e.getX());
            node2.setLayoutY(e.getY());
        }
    }
    @FXML private void onGraphSpaceReleased(MouseEvent e) {
    ...
    @FXML public void  resetNetwork(ActionEvent actionEvent) {
        ...
        Platform.runLater( () -> {
            ...
            for(Node n : graphSpace.getChildren()){
                if(n instanceof VisNode){
                    n.pseudoClassStateChanged(Controller.ps_currentNode, false);
                    n.pseudoClassStateChanged(Controller.ps_neighbourNode, false);
                    setNodeStyleClass((VisNode) n, Controller.DEFAULT);
                }
                if(n instanceof Edge){
                    n.getStyleClass().removeAll("path");
                }
            }
            listA.getItems().clear();
            listB.getItems().clear();
    ...
    private void onNodeDragDetected(MouseEvent e, VisNode n) {
        if(e.isPrimaryButtonDown()){
            n.startFullDrag();
            node1 = n;
            node2 = createAndAddNode(n.getLayoutX() + e.getX() + n.getTranslateX(),
                                     n.getLayoutY() + e.getY() + n.getTranslateY());
            arrow = createArrow(node1, node2);
            node2.getStyleClass().add("dragged");
            arrow.getStyleClass().add("dragged");
            node2.toBack();
     ...
     * */
    private void onNodeDragged(MouseEvent e, VisNode n) {
        if(node2 != null){
    ...
    private VisNode createAndAddNode(double x, double y) {
        ...
        graphSpace.getChildren().add(vNode); //add node to Fx Graph Space
        addToAccordian(vNode);
        vNode.setOnMousePressed(z -> nodeMousePressed(z, vNode));
        vNode.setOnDragDetected(z -> onNodeDragDetected(z, vNode));
        vNode.setOnMouseDragged(z -> onNodeDragged(z, vNode));
        vNode.setOnMouseReleased(z -> onNodeReleased(z, vNode));
    ...
    Edge createArrow(VisNode node1, VisNode node2){
        Edge arrow = new Edge(node1, node2);
        arrow.distanceVisibleProperty().bind(chbShowDistance.selectedProperty());
        arrow.weightVisibleProperty().bind(chbShowWeight.selectedProperty());

        node1.edges.add(arrow);
        node2.edges.add(arrow);
        graphSpace.getChildren().add(arrow);
        return arrow;
    }
